...
Secrets: `BEUI_PRO_TOKEN` lives in 1Password and is used through the 1Password browser extension in Zen. Names in context, values never. Never commit the token, a resolved registry URL that carries it, or an `.npmrc` that contains it.
