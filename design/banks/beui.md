...
Secrets: `BEUI_PRO_TOKEN` lives only in the Workspace Vault or Computer. Names in context, values never. Never commit the token, a resolved registry URL that carries it, or an `.npmrc` that contains it.
