...
| `ui-frosted-grain.patch` | chrome UI | frosted-glass grain on the frame/shell: native frame theme tint -> 0.65 alpha (35% transparency over the macOS vibrancy blur), plus a deterministic 160px luminance-noise tile composited as a `grainOverlay` at 0.08 opacity above the tint; pairs with `resources/zenmium/internal/` CSS grain+glass treatment on Zenmium-owned surfaces (macOS frame only — upstream materials seam has no Windows/Linux equivalent) |
