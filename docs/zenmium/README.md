...
Status: planned. Base repository: `FujiwaraChoki/mori-browser` (MIT, macOS, a SwiftUI/AppKit chrome compiled into ungoogled-chromium). This planning set lives in `nicharacci/zenmium`, the factory workspace that builds the product.
...
| --- | --- | --- | --- |
| D1 | Base | Fork `FujiwaraChoki/mori-browser` and keep its ungoogled-chromium build and Objective-C++ bridge (MIT) | A real Chromium engine with the Arc sidebar, spaces, folders, and extension bridge already present |
...
1. Product fork home. Where the Mori fork lives and the value of `FACTORY_REPO` for the factory workspace.
2. AppKit host choice. A WKWebView chrome surface, or a Mori Chromium WebContents dedicated to the chrome, separate from page content.
