...
Goalpost Factory Template for Goalpost Code company workspaces. Root agent is CAO (DevOps and BizOps hats) on the eve filesystem. Engineering work uses CAOstack (pstack overlay, no solvys-skills dump). Stations stay in eve directories: classifier (intake), analyst (plan), implementer (poteto-agent-1 or 2), reviewer (supervisor). Unattended runs still open a draft PR on `FACTORY_REPO`. Always-on context is this file plus `.cursor/rules/caostack-core.mdc`. OS pack stub: `.goalpost/os/manifest.json` (`updates.enabled` defaults to true; no live puller yet). Run `bash caostack/install.sh` before expecting pstack playbooks.

The whole agent is defined under `agent/`. eve discovers capabilities from the filesystem. See [`ARCHITECTURE.md`](./.github/ARCHITECTURE.md) and [`caostack/lock.json`](./caostack/lock.json).

## Setup & commands

```bash
bash caostack/install.sh   # vendor pstack only. never solvys-skills
